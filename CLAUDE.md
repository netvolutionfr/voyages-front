...
**Account erasure** (`src/pages/profil/AccountDeletionCard.tsx`, rendered on `/profil/donnees`): two-step flow — consent checkbox → `POST /me/delete-request` (emails a 6-digit code, ~10 min TTL) → `InputOTP` entry (`autoComplete="one-time-code"`) → `DELETE /me { otp }`. On `204` it calls `clearAuth()` then `window.location.assign("/login")` (full reload — the backend already cleared the refresh cookie, so the Refine `logout` flow must NOT be used). Backend error codes (`invalid_otp`, `too_many_requests`, `student_self_erasure_forbidden`, `last_active_admin`, `active_trip_registration`, `guardian_of_active_student`) map to a front-side message dictionary with a generic fallback — raw server messages are never rendered. `invalid_otp` keeps the OTP step; other (403/409) codes reset to the initial state since retrying can't succeed. Resend has a 60s client-side cooldown (mirrors the backend 429). The whole card is replaced by an explanatory message for `STUDENT` role (read once from the JWT; UI-only defense, the backend enforces the 403).
